...
from .subtitles import Caption, excessive_tail_violations, speech_gap_violations
...
def subtitle_qa(captions:list[Caption], words:list[WordTiming], duration:float)->dict:
    ...
    tails=excessive_tail_violations(captions,words)
    return {"status":"PASS" if not bad and not gaps and not tails else "FAIL","invalid":len(bad),"speech_gaps":len(gaps),"excessive_tail":len(tails)}
